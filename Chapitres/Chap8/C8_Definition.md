...
Supposons que l'on veuille estimer un bloc \\( v \\) centré au point \\( \\mathbf{x}\_0 \\). Notons \\( Z\_v \\) la vraie valeur (inconnue) de ce bloc et \\( Z\_v^\* \\) l'estimateur que l'on obtient.
...
Z\_v^\* = \\sum\_{i=1}^n \\lambda\_i Z(\\mathbf{x}\_i)

\\]



où les \\( Z\_i \\) désignent les variables aléatoires correspondant aux points échantillons.
...
\\sigma\_K^2 = \\mathrm{Var}(Z\_v - Z\_v^\*)
...
\\sigma\_K^2 = \\mathrm{Var} \\left( Z\_v - \\sum\_{i=1}^n \\lambda\_i Z(\\mathbf{x}\_i) \\right)
...
\\mathbb{E}\[Z\_v^\*] = \\sum\_{i=1}^n \\lambda\_i \\mathbb{E}\[Z(\\mathbf{x}\_i)] = \\mathbb{E}\[Z\_v]
...
\\mathcal{L}(\\lambda\_1, \\dots, \\lambda\_n, \\mu) = \\mathrm{Var}\\left(Z\_v - \\sum\_{i=1}^n \\lambda\_i Z(\\mathbf{x}\_i)\\right) + \\mu \\left( \\sum\_{i=1}^n \\lambda\_i - 1 \\right)
...
Le minimum est atteint lorsque toutes les dérivées partielles par rapport à chacun des \\( \\lambda\_i \\) et par rapport à \\( \\mu \\) s'annulent. Ceci conduit au \*\*système de krigeage ordinaire\*\* :
...
\\sum\_{j=1}^n \\lambda\_j C(\\mathbf{x}\_i - \\mathbf{x}\_j) + \\mu = C(\\mathbf{x}\_i - \\mathbf{x}\_0), \& i = 1, \\dots, n \\\\
...
\\sigma\_K^2 = \\mathrm{Var}(Z\_v - Z\_v^\*) = \\sum\_{i=1}^n \\lambda\_i C(\\mathbf{x}\_i - \\mathbf{x}\_0) + \\mu
...
et en remarquant que \\( \\sum \\lambda\_i = 1 \\), on a alors :



\\\[

\\sum\_{j=1}^n \\lambda\_j \\gamma(\\mathbf{x}\_i - \\mathbf{x}\_j) - \\mu = \\gamma(\\mathbf{x}\_i - \\mathbf{x}\_0), \\quad i = 1, \\dots, n
...
Z\_v^\* = m + \\sum\_{i=1}^n \\lambda\_i (Z(\\mathbf{x}\_i) - m)

\\]



Tout comme pour le krigeage ordinaire, on écrit la \*\*variance d'estimation\*\* et on substitue l'expression précédente pour l’estimateur \\( Z\_v^\* \\). On obtient :



\\\[

\\sigma\_K^2 = \\mathrm{Var} \\left( Z\_v - m - \\sum\_{i=1}^n \\lambda\_i (Z(\\mathbf{x}\_i) - m) \\right)

\\]



En dérivant cette expression par rapport à chacun des \\( \\lambda\_i \\), on trouve le \*\*système de krigeage simple\*\* :



\\\[

\\sum\_{j=1}^n \\lambda\_j C(\\mathbf{x}\_i - \\mathbf{x}\_j) = C(\\mathbf{x}\_i - \\mathbf{x}\_0), \\quad i = 1, \\dots, n
...
\\sigma\_K^2 = \\sigma^2 - \\sum\_{i=1}^n \\lambda\_i C(\\mathbf{x}\_i - \\mathbf{x}\_0)
...
\- La variance de krigeage simple est \*\*toujours inférieure\*\* à celle du krigeage ordinaire, car aucune contrainte n'est imposée sur les poids \\( \\lambda\_i \\). Toutefois, elle \*\*requiert la connaissance de la moyenne\*\* \\( m \\).
...
\- Il est \*\*impossible de formuler le système de krigeage simple directement en termes de variogramme\*\*, puisque l’on n’a pas \\( \\sum \\lambda\_i = 1 \\).
...
\- \\( \\boldsymbol{\\lambda} \\) est le vecteur des poids \\( \\lambda\_i \\),
...
Z\_v^\* = m + \\boldsymbol{\\lambda}^\\top ( \\mathbf{Z} - m \\mathbf{1} )
...
\\sigma\_K^2 = \\sigma^2 - \\boldsymbol{\\lambda}^\\top \\mathbf{c}\_0
