// scripts/geostat-js/widgets/c05_triangles.js
// -----------------------------------------------------------------------------
// Widget « Méthode des triangles (TIN) » (C05-W2)
// Source de vérité : geostat_polymtl.conventional.triangles.interpolation_triangulaire
// -----------------------------------------------------------------------------

import { Widget } from '../widget-base.js';
import { GRF, cm, cmA, del, bary, computeError } from './c05_lib.js';
import { afficherChargementJusquaPret } from '../pyodide_setup.js';

const W=430, H=340;

export default class C05Triangles extends Widget {
  render() {
    const grad = Array.from({length:11},(_,i)=>{const c=cm(i/10);return `rgb(${c[0]},${c[1]},${c[2]})`;}).join(',');
    this.el.insertAdjacentHTML('beforeend', `
      <div style="display:flex;gap:4px;margin-bottom:8px;">
        <canvas id="${this.el.id}_cT" width="${W}" height="${H}"
          style="border:1px solid #ccc;border-radius:4px;cursor:crosshair;flex:1;max-width:50%"></canvas>
        <canvas id="${this.el.id}_cE" width="${W}" height="${H}"
          style="border:1px solid #ccc;border-radius:4px;cursor:crosshair;flex:1;max-width:50%"></canvas>
      </div>
      <div style="display:flex;gap:16px;font-size:0.78rem;font-family:'JetBrains Mono',monospace;color:#666;margin-bottom:6px;">
        <span>◀ <b>Réalité</b> (champ gaussien)</span>
        <span style="margin-left:auto"><b>Estimation</b> (triangles / TIN) ▶</span>
      </div>
      <div style="display:flex;align-items:center;gap:8px;font-size:.72rem;color:#555;margin:0 2px 8px;">
        <span style="font-weight:600;white-space:nowrap;">Teneur (%)</span>
        <div style="flex:1;">
          <div style="height:12px;border:1px solid #bbb;border-radius:3px;background:linear-gradient(to right, ${grad});"></div>
          <div style="display:flex;justify-content:space-between;font-size:.66rem;color:#777;margin-top:1px;">
            <span>0</span><span>1</span><span>2</span><span>3</span><span>4</span><span>5</span><span>6</span><span>7</span><span>8</span><span>9</span><span>10</span>
          </div>
        </div>
      </div>
      <div class="gw-controls">
        <label>Mode :</label>
        <select id="${this.el.id}_mode">
          <option value="interp">Interpolation barycentrique</option>
          <option value="flat">Teneur moyenne par triangle</option>
        </select>
        <button id="${this.el.id}_btnAdd">+ 8 pts</button>
        <button id="${this.el.id}_btnRst">Nouveau champ</button>
        <span id="${this.el.id}_err" style="min-width:200px;font-size:0.78rem">—</span>
      </div>
      <div class="gw-controls" style="margin-top:2px;background:#e8edf2;">
        <span style="font-weight:600;font-size:.74rem;">Champ :</span>
        <select id="${this.el.id}_gm" style="font-size:.72rem;"><option value="exponentiel">Exponentiel</option><option value="spherique">Sphérique</option><option value="gaussien">Gaussien</option></select>
        <div class="gw-slider"><label style="font-size:.72rem;">aₓ:</label><input type="range" id="${this.el.id}_px" min="20" max="250" value="130" style="width:60px;"><span id="${this.el.id}_pxv" style="font-size:.7rem;">130</span></div>
        <div class="gw-slider"><label style="font-size:.72rem;">aᵧ:</label><input type="range" id="${this.el.id}_py" min="20" max="250" value="130" style="width:60px;"><span id="${this.el.id}_pyv" style="font-size:.7rem;">130</span></div>
        <div class="gw-slider"><label style="font-size:.72rem;">c₀:</label><input type="range" id="${this.el.id}_ng" min="0" max="50" value="0" style="width:50px;"><span id="${this.el.id}_ngv" style="font-size:.7rem;">0%</span></div>
      </div>
      <div class="gw-probe" id="${this.el.id}_probe">Survolez l'estimation pour voir les poids barycentriques.</div>
    `);

    const id = this.el.id;
    this.ctxT = document.getElementById(`${id}_cT`).getContext('2d');
    this.ctxE = document.getElementById(`${id}_cE`).getContext('2d');
    this.cvsE = document.getElementById(`${id}_cE`);
    this.modeSel = document.getElementById(`${id}_mode`);
    this.errEl   = document.getElementById(`${id}_err`);
    this.probeEl = document.getElementById(`${id}_probe`);
    this.pts = []; this.tris = [];

    const cvsT = document.getElementById(`${id}_cT`);
    this.on(cvsT,      'click', e => this._handler(e));
    this.on(this.cvsE, 'click', e => this._handler(e));
    this.on(this.cvsE, 'mousemove', e => this._probe(e));
    this.on(this.modeSel, 'change', () => this._drawEstim());
    this.on(document.getElementById(`${id}_btnAdd`), 'click', () => this._addRandom(8));
    this.on(document.getElementById(`${id}_btnRst`), 'click', () => this._reset());
    for (const [suf, disp] of [['px','pxv'],['py','pyv']]) {
      this.on(document.getElementById(`${id}_${suf}`), 'input', e => { document.getElementById(`${id}_${disp}`).textContent = e.target.value; });
    }
    this.on(document.getElementById(`${id}_ng`), 'input', e => { document.getElementById(`${id}_ngv`).textContent = e.target.value+'%'; });
    // Changer le modèle, les portées ou la pépite RÉGÉNÈRE le champ avec la même
    // graine : seule la covariance change, pas le tirage aléatoire. Les points
    // déjà échantillonnés gardent leur position et relisent la nouvelle réalité.
    this.seed = Math.floor(Math.random() * 1e9);
    this.on(document.getElementById(`${id}_gm`), 'change', () => this._regenerer());
    for (const suf of ['px', 'py', 'ng']) {
      this.on(document.getElementById(`${id}_${suf}`), 'input', () => this._regenerer());
    }

    afficherChargementJusquaPret(this.el).then(async () => {
      this.grf = await this._mkGrf();
      this._drawTruth(); this._drawEstim();
    });
  }

  async _mkGrf() {
    const id = this.el.id;
    return await GRF.create(W, H, {
      modele:   document.getElementById(`${id}_gm`).value,
      portee_x: +document.getElementById(`${id}_px`).value,
      portee_y: +document.getElementById(`${id}_py`).value,
      pepite:   +document.getElementById(`${id}_ng`).value / 100,
      seed:     this.seed,
    });
  }

  _estim(x, y) {
    const mode = this.modeSel.value;
    for (const [a,b,c] of this.tris) {
      // bary(x, y, A, B, C) attend des POINTS {x,y} et renvoie [l1, l2, l3].
      const bc = bary(x, y, this.pts[a], this.pts[b], this.pts[c]);
      if (!bc) continue;
      // À l'intérieur du triangle, les 3 coords barycentriques sont ≥ 0.
      if (bc[0] < -1e-9 || bc[1] < -1e-9 || bc[2] < -1e-9) continue;
      return mode==='interp'
        ? bc[0]*this.pts[a].t + bc[1]*this.pts[b].t + bc[2]*this.pts[c].t
        : (this.pts[a].t+this.pts[b].t+this.pts[c].t)/3;
    }
    return null;
  }

  _drawTruth() {
    this.grf.drawTo(this.ctxT);
    for (const p of this.pts) {
      this.ctxT.beginPath(); this.ctxT.arc(p.x,p.y,4,0,Math.PI*2);
      this.ctxT.fillStyle='#fff'; this.ctxT.fill(); this.ctxT.strokeStyle='#000'; this.ctxT.lineWidth=1.5; this.ctxT.stroke();
    }
    this.ctxT.fillStyle='rgba(255,255,255,0.7)'; this.ctxT.fillRect(4,4,70,18);
    this.ctxT.fillStyle='#333'; this.ctxT.font='bold 10px JetBrains Mono'; this.ctxT.fillText('RÉALITÉ',8,16);
  }

  _drawEstim() {
    const cE=this.ctxE, pts=this.pts, mode=this.modeSel.value;
    cE.clearRect(0,0,W,H); cE.fillStyle='#f5f3ef'; cE.fillRect(0,0,W,H);
    if (pts.length < 3) {
      for (const p of pts) {
        cE.beginPath(); cE.arc(p.x,p.y,4,0,Math.PI*2); cE.fillStyle='#fff'; cE.fill();
        cE.strokeStyle='#222'; cE.lineWidth=1.5; cE.stroke();
        cE.fillStyle='#222'; cE.font='10px JetBrains Mono'; cE.fillText((p.t*10).toFixed(1)+'%',p.x+6,p.y-5);
      }
      cE.fillStyle='#aaa'; cE.font='13px Source Serif 4'; cE.textAlign='center';
      cE.fillText(`${3-pts.length} point(s) de plus`,W/2,H/2); cE.textAlign='start'; return;
    }
    if (mode==='interp') {
      const s=3, img=cE.createImageData(W,H), d=img.data;
      for (let y=0;y<H;y+=s) for (let x=0;x<W;x+=s) {
        const v=this._estim(x,y); if(v===null) continue;
        const cl=cm(v);
        for (let dy=0;dy<s&&y+dy<H;dy++) for (let dx=0;dx<s&&x+dx<W;dx++) {
          const o=((y+dy)*W+(x+dx))*4; d[o]=cl[0]; d[o+1]=cl[1]; d[o+2]=cl[2]; d[o+3]=200;
        }
      }
      cE.putImageData(img,0,0);
    } else {
      for (const [a,b,c] of this.tris) {
        const av=(pts[a].t+pts[b].t+pts[c].t)/3;
        cE.beginPath(); cE.moveTo(pts[a].x,pts[a].y); cE.lineTo(pts[b].x,pts[b].y); cE.lineTo(pts[c].x,pts[c].y); cE.closePath();
        cE.fillStyle=cmA(av,.75); cE.fill();
      }
    }
    cE.strokeStyle='rgba(0,0,0,0.2)'; cE.lineWidth=0.8;
    for (const [a,b,c] of this.tris) { cE.beginPath(); cE.moveTo(pts[a].x,pts[a].y); cE.lineTo(pts[b].x,pts[b].y); cE.lineTo(pts[c].x,pts[c].y); cE.closePath(); cE.stroke(); }
    for (const p of pts) {
      cE.beginPath(); cE.arc(p.x,p.y,4,0,Math.PI*2); cE.fillStyle='#fff'; cE.fill();
      cE.strokeStyle='#000'; cE.lineWidth=1.5; cE.stroke();
      cE.fillStyle='#222'; cE.font='10px JetBrains Mono'; cE.fillText((p.t*10).toFixed(1)+'%',p.x+6,p.y-5);
    }
    cE.fillStyle='rgba(255,255,255,0.7)'; cE.fillRect(W-82,4,78,18);
    cE.fillStyle='#333'; cE.font='bold 10px JetBrains Mono'; cE.fillText('ESTIMATION',W-78,16);
    const err=computeError(this.grf,(x,y)=>this._estim(x,y),W,H,6);
    this.errEl.textContent=`Biais: ${(err.biais*10).toFixed(3)}% | RMSE: ${(err.rmse*10).toFixed(3)}%`;
  }

  _probe(e) {
    if (this.pts.length < 3) return;
    const r=e.target.getBoundingClientRect();
    const px=(e.clientX-r.left)*(W/r.width), py=(e.clientY-r.top)*(H/r.height);
    for (const [a,b,c] of this.tris) {
      const bc = bary(px, py, this.pts[a], this.pts[b], this.pts[c]);
      if (!bc) continue;
      if (bc[0] < -1e-9 || bc[1] < -1e-9 || bc[2] < -1e-9) continue;
      const ip=bc[0]*this.pts[a].t+bc[1]*this.pts[b].t+bc[2]*this.pts[c].t;
      this.probeEl.innerHTML=`λ=(${bc[0].toFixed(2)}, ${bc[1].toFixed(2)}, ${bc[2].toFixed(2)}) | Interp: <b>${(ip*10).toFixed(2)}%</b> | Réalité: <b>${(this.grf.at(px,py)*10).toFixed(2)}%</b>`;
      return;
    }
    this.probeEl.innerHTML='Hors triangulation.';
  }

  _addPt(x, y) { this.pts.push({x,y,t:this.grf.at(x,y)}); this.tris=del(this.pts); this._drawTruth(); this._drawEstim(); }

  _handler(e) {
    const r=e.target.getBoundingClientRect();
    this._addPt((e.clientX-r.left)*(W/r.width),(e.clientY-r.top)*(H/r.height));
  }

  _addRandom(n) { for(let i=0;i<n;i++) this._addPt(30+Math.random()*(W-60),30+Math.random()*(H-60)); }

  async _reset() { this.seed=Math.floor(Math.random()*1e9); this.pts=[]; this.tris=[]; this.grf=await this._mkGrf(); this._drawTruth(); this._drawEstim(); }

  /** Régénère le champ (même graine) après un changement de covariance. */
  _regenerer() {
    clearTimeout(this._tRegen);
    this._tRegen = setTimeout(async () => {
      const jeton = (this._jetonRegen = (this._jetonRegen || 0) + 1);
      let grf;
      try { grf = await this._mkGrf(); }
      catch (e) { this.afficherErreur('Simulation du champ : ' + (e && e.message ? e.message : e)); return; }
      if (jeton !== this._jetonRegen || this._destroyed) return;   // réglage plus récent, ou widget fermé
      this.grf = grf;
      for (const p of this.pts) p.t = grf.at(p.x, p.y);
      this._drawTruth(); this._drawEstim();
    }, 250);
  }
}
